...
function BussinessAccounts() {
  ...
  const [pricingDialogOpen, setPricingDialogOpen] = useState<string | null>(null); 
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      pincode:'',
      state:'',
      image:'https://www.logoshape.com/wp-content/uploads/2024/09/zepto-logo-vector_logoshape.png'
    },
  });
    ...
    <div className="flex justify-end">
     <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button variant="default" type="button">Add Account</Button>
        </DialogTrigger>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Create Bussiness Account</DialogTitle>
            <DialogDescription>
            Come join with us and grow your business. Fill in the details to get started!
            </DialogDescription>
          </DialogHeader>
...
{list.length>0&&list.map((item:BussinessAccount)=>(
  <Card key={item._id}>
  <CardHeader>
  ...
  <CardFooter>
    { !item.isPricing && <Dialog open={pricingDialogOpen === item._id} onOpenChange={(isOpen) => setPricingDialogOpen(isOpen ? item._id : null)}>
        <DialogTrigger asChild>
          <Button variant="default" type="button">Add Pricings</Button>
        </DialogTrigger>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Add Pricings</DialogTitle>
            <DialogDescription>
           Add pricings for available vehicle Models.
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
      </Dialog>}

    {item.isPricing&& <Button type="button" variant='default'>Update Pricings</Button>}
  </CardFooter>
</Card>
